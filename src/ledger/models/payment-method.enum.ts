 ...
 * and `AdminPaymentAttempt.method`). `CASH` (GOS-87) and `MERCADOPAGO`
 * (GOS-85) are both real, written values today. `MERCADOPAGO` replaced the
...
registerEnumType(PaymentMethod, {
  name: 'PaymentMethod',
  description:
    'Which payment method an Engagement is/was using — the COLLECTOR, not the instrument (see PaymentAttemptType for HOW, e.g. credit card vs account money). CASH (GOS-87) and MERCADOPAGO (GOS-85) are both real. A future second provider adds a new value here.',
});
