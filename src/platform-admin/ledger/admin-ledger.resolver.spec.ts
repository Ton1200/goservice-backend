...
import { AdminLedgerResolver } from './admin-ledger.resolver';

/**
 * Wiring-only tests — same reasoning as `AdminAppointmentsResolver`'s own
 * `admin-appointments.resolver.spec.ts`: the actual BEHAVIOR of
 ...
 */
describe('AdminLedgerResolver wiring', () => {
  ...
  it('requires Permission.LEDGER_READ on adminLedgerEntries — its own dedicated permission', () => {
    ...
    const handler = resolverPrototype.adminLedgerEntries;
